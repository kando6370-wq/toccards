...
({String r, String g, String b}) hashScanCardRgb(
  Uint8List pixels, {
  required int width,
  required int height,
}) {
  if (width <= 0 || height <= 0 || pixels.length != width * height * 3) {
    throw ArgumentError('The corrected RGB card image is invalid.');
  }
  final channels = List.generate(3, (_) => Uint8List(width * height));
  for (var index = 0; index < width * height; index += 1) {
    for (var channel = 0; channel < 3; channel += 1) {
      channels[channel][index] = pixels[index * 3 + channel];
    }
  }
  final hashes = [
    for (final channel in channels)
      encodeScanPhash(letterboxScanChannelPillowLanczos(
