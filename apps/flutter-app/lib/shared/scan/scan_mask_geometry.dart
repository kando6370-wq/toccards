...
List<ScanImagePoint> orderScanCorners(List<ScanImagePoint> points) {
  ...
  final rotated = [
    for (var index = 0; index < 4; index += 1) ordered[(start + index) % 4],
  ];
...
List<ScanImagePoint> _approximateClosedPolygon(
  ...
  return [...first.take(first.length - 1), ...second.take(second.length - 1)];
}
...
List<ScanImagePoint> _approximatePolyline(
  ...
  while (ranges.isNotEmpty) {
    ...
    for (var index = start + 1; index < end; index += 1) {
      final distance = _pointLineDistance(
        points[index],
        points[start],
        points[end],
      );
...
_Rectangle? _minimumAreaRectangle(List<ScanImagePoint> hull) {
  ...
  for (var index = 0; index < hull.length; index += 1) {
    ...
    bestArea = area;
    ScanImagePoint restore(double x, double y) =>
        ScanImagePoint(x * cosine - y * sine, x * sine + y * cosine);
    best = _Rectangle(
      [
