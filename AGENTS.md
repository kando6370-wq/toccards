...
- `deploy:dev` 已改为构建 Linux 发布包并通过显式 SSH 目标发布，`deploy:dry-run:dev` 仅打包，不连接服务器；prod 继续使用原 Wrangler 发布入口。Linux 发布须先通过环境/数据库/识别预检，再备份已有数据库；标准与离线 Compose 均以 PostgreSQL 18 和原有 `PGDATA=/var/lib/postgresql/data` 为契约，不自动进行大版本升级。2026-09-16 整改已通过 `75c0ec4` 合入 dev，kd201 的 dev 监听器已自动发布该提交并通过受控扫描、扣次和本地收藏写库验证，ledger 保持 13 项（0000—0012）。API 与 watcher 的代理配置分别存放在各自私有环境文件；旧 CF dev 退役、公网 Apple 回调和真机仍待验收，详情见当前版本 `VERIFICATION.md`。
