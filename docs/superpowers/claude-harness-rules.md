...
- `docs/tcg-card/05-plan/dev-plan.md` 是只读计划真源；`execution-status.md` 负责呈现当前执行态、计划状态覆盖层与执行日志
- 若任务对应 `dev-plan.md`，任务标题必须带 `[Mx-y]` 或 `[TBD Mx-A]` 前缀；无前缀任务只写执行日志，不更新计划状态覆盖层
- 复杂任务还应补充：已完成事项、验证结果、剩余风险
- 只有在完成门验证通过后，任务才能写成 `completed`；验证失败时必须保留未完成状态并记录失败
