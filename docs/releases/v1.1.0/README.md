...
订单美元金额的财务事实已新增 `0033_billing_exchange_rate_snapshot.sql`：新订单固化现有汇率服务的 rate、来源、时间、版本和舍入口径；汇率不可证明时保留空值，不阻断订单或伪造金额。`0034_billing_auto_renew_snapshot.sql` 进一步固化订单发生时可证明的自动续订状态；后续链路状态变化不覆盖历史，无法证明的历史/Fresh Purchase/Restore 订阅订单保持空值，Lifetime 固定为否。升级/降级通知只更新下一 Product ID、不建单；重订阅以 Apple `originalTransactionId` 购买链判断首次付款或同链续费，不仅依赖 `transactionReason`。自动化完整迁移链已通过 `0000` 至 `0034`；Wrangler 空库及远程 dev 副本证据仍只到 `0033`。远程 dev/prod 当前待执行 `0025` 至 `0034`，尚未进行远程写入。
