...
服务端已覆盖 Fresh Purchase 和 Restore 的 `VERIFIED_ACTIVE` 写链。App 通过 iOS 原生桥接完成 `AppStore.sync()` + verified `Transaction.currentEntitlements` 的本机 Restore：配置 SKU 命中即即时 Premium、同步成功后的空结果为 Not Found、配置 SKU unverified/25 秒读取超时为 Failed。`AppStore.sync()` 取消、账号验证失败或其他同步错误均停止本次 Restore，不得继续读取设备残留 entitlement 并误报 Success；其中用户取消只结束 Restore Loading、保持操作前权益且不显示结果反馈，账号验证失败及其他真实错误仍进入 Failed。本机 Success 后后台执行 App Attest 注册/assertion 与 JWS 同步；同步失败不撤销本机 Premium，但不创建 session grant。Notifications V2 与 Apple Server API 校正已覆盖生命周期失效：Billing Retry/Expired 将同链 grant 标记 expired，Refund/Revoke 标记 revoked，Grace 保持 active；该失效归约是服务端链路状态，不复用客户端 Fresh Purchase/Restore 接口的 `VERIFIED_INACTIVE` 响应。
...
| 权益 Unknown | 先刷新，25 秒后仍 Unknown 则返回 `ENTITLEMENT_SYNC_REQUIRED` | 不得消费 Free Quota | 不得伪装 No Data |
...
- 只有目录中存在非空 `card_ref/name/set_name`、可映射为 `object_type=tcg`，且可构造 Card Detail 基础响应的 Matched 结果才结算为 consumed；No Match、目录基础信息不完整以及上传、识别、解析或持久化失败均结算为 released。识别开始前的 Free 预占不等于扣次，released 会恢复该次预占；完成后的识别业务结果可由同 session、同 request ID 幂等重放，不能重复扣次或重复释放，重放响应中的 Quota 按当前服务端账本刷新，不沿用可能过时的结算前快照。
- Matched 候选响应必须包含 `card_ref`、`name`、`set_name`、`object_type`，Flutter 缺少任一必填字段时显式失败，不得只凭名称进入成功状态。价格不是成功条件；基础信息和详情可用但没有市场价格时仍为 Matched，沿用无价格展示。
...
- Flutter 不再持久化或读取本地 Scan Quota，使用本机 Premium 或服务端 `unlimited=true` 的合并结果控制 Scan 展示和额度拦截；本机为 Free 时在页面生命周期内至少复核一次 StoreKit，服务端已确认 Unlimited 时请求继续携带同步保护。Gallery 超额与 Failed Retry 可保留原图转 Waiting；单张 Capture quota=0 不保留 Item；Waiting 删除不打开 Paywall，卡体打开 Paywall；Done 仅在存在 Matched 且无 Processing 时可用。同一 Capture/Gallery 批次的 Processing 全部结束后只刷新一次服务端 Quota，再按权威 `remaining` 顺序递补 Waiting；刷新期间新的 Capture/Gallery 使用等待提示，不得按旧 `remaining=0` 误开 Paywall。Purchase/Restore 后只有服务端确认 `unlimited=true` 才恢复全部 Waiting；`ENTITLEMENT_SYNC_REQUIRED` 保留现场且不转 Failed、不弹 Free Paywall。Processing 删除后继续在后台等待服务端结算但不重插 UI，Waiting Paywall 只在 typed success 且原 Queue 仍有效时恢复。
