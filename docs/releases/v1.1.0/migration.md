...
本文件只记录迁移设计。除上述隔离空库验证外，尚未应用到开发者常用 local、远程 dev 或 prod 数据库。
...
该迁移是向后兼容新增。回滚前必须先停止 challenge/verify 接口写入，再删除两个新表；SQLite 已增加的 `state_effective_at` 不在原地回滚，旧代码会忽略该 nullable 列。除隔离空库验证外，尚未应用到常用 local、远程 dev 或 prod。

## Workers 运行兼容性

Apple JWS 验签采用官方 `@apple/app-store-server-library`，`apps/workers-api/wrangler.toml` 启用 `nodejs_compat`。该配置同时影响 dev 与 prod Worker；当前仅完成两环境 dry-run 打包验证，尚未部署。
...
回滚前必须先停用 App Attest challenge/register/restore 三个接口，再导出需要保留的重放审计数据，最后删除两张表。回滚会使新 session 无法通过 Restore 建立服务端 grant，但不影响 StoreKit 本机 Restore 结果。当前已通过真实 Miniflare D1 的 session 隔离、单次消费、counter 和 grant 写入测试；除隔离空库验证外，尚未应用到常用 local、远程 dev 或 prod。
...
当前已通过真实 Miniflare D1 的原文留存、幂等建单、乱序保护、失败恢复与环境隔离测试；除隔离空库验证外，`0029` 尚未应用到常用 local、远程 dev 或 prod。
...
当前真实 Miniflare D1 已验证试用为 0、付费顺序、晚到交易重排、重复交易幂等、退款保序、Restore 写入、精确订单查询、动态选项、XLSX 和 inbox 失败详情；除隔离空库验证外，`0032` 尚未应用到常用 local、远程 dev 或 prod。
...
当前真实 Miniflare D1 已验证最后 1 次额度的多 session 并发、预占/结算幂等、技术失败释放、Premium 不扣 Free 和租约接管；除隔离空库验证外，`0030` 尚未应用到常用 local、远程 dev 或 prod。
