...
- PRD 验收 110 的重启边界已有直接回归保护：Secure Storage entitlement cache 只序列化 Apple 已验证权益事实，不保存 Paywall、Restore Loading/Failed、Subscription Success、Purchase Pending 或 `blocked_action` 等瞬态 UI/业务恢复状态。该定向测试 7 项及文件静态分析通过。
