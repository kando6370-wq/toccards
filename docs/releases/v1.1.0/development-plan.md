...
- Revenue 持久化边界已强化：reported transaction ID 会过滤空值，pending Map key 必须与记录内 `transaction_id` 完全一致，金额必须为有限数值；损坏本地记录不能绕过幂等、形成永久待重试项或上报 NaN/Infinity。Restore、Pending、Unverified 仍不记录收入。Revenue 定向 5 项及实现/测试文件静态分析通过。
