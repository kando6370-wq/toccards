...
| Billing 数据 | `0025` 至 `0034` 已覆盖购买链、交易、session grant、原始通知 inbox、生命周期、订单业务事实、USD 汇率及自动续订订单快照；自动化完整迁移链已通过 `0000-0034`。Wrangler 空库和远程 dev 只读副本仍只实际验证到 `0033` | 远程 dev/prod 均待执行 `0025` 至 `0034`；`0034` 尚未做 Wrangler/真实历史副本预演；缺 Sandbox/TestFlight 实单验收 |
