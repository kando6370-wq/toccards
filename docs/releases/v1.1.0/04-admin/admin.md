...
环境筛选按部署构建模式设置默认值：`production` 默认“正式”（`Production`），`development` 和 `linux` 默认“测试”（`Sandbox`）。首次查询直接使用该默认值，重置时清空其他条件并恢复对应环境；仍可手动切换环境或清空为“全部”。

安装时间和订单时间只选择年月日，输入框格式为 `YYYY-MM-DD`，不提供时分秒选择。前端提交纯日期，后端按 UTC+0 将开始日期扩展为当天 `00:00:00.000`、结束日期扩展为当天 `23:59:59.999`，包含结束日期最后一秒内的全部记录；列表与 XLSX 导出使用相同边界。清空或重置时移除对应时间筛选。
...
环境默认值与订单统计一致：正式部署默认“正式”，测试部署（含 Linux）默认“测试”，首次查询和重置均使用对应默认值；支持手动切换或清空为“全部”。

创建时间只选择年月日，输入框格式为 `YYYY-MM-DD`。前端提交纯日期，后端按 UTC+0 查询开始日期 `00:00:00.000` 至结束日期 `23:59:59.999` 的闭区间；清空或重置时移除时间筛选。
...
- 旧 CF dev/prod 共库阶段的 `0011_app_version_environment.sql` 执行顺序为：先将既有规则一次性复制为独立配置，已有独立配置不覆盖；仅发布当时的 dev Worker 时可先初始化 development 两条键，prod 继续读取旧键。2026-09-08 已完成该历史 dev 阶段。迁移与对应 Worker 切换期间暂停该环境版本配置编辑；新 Worker 不回退旧键，回滚只能使用支持独立键的 Worker。后续 prod 迁移与 Worker 切换需独立核验，不得把现有 Linux dev 的独立数据库当作共库迁移目标；详见[版本控制验收](../05-delivery/VERIFICATION.md)。
