...
扫描识别使用端侧 RTMDet-Ins 与 PE-Core-T16，主 Worker 经 `VECTOR_RECOGNITION` Service Binding 调用内部 `recognize-vec`；该链路已从 dev-wxy 合入 dev。图片仍只存私有 R2，内部服务只收向量；Queue、额度、目录与资产写入保留现有边界，详见[扫描识别链路](../01-flows/scan-recognition.md)。
