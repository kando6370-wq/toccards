...
以上 bindings 属于 prod。dev Linux 使用独立 PostgreSQL；标准 Compose 默认 `postgres:18-alpine`，离线镜像安装 PostgreSQL 18，两者显式设置 `PGDATA=/var/lib/postgresql/data` 保留现有卷路径。2026-09-20 回读 kd201 当前 API release `dev@9488a15`：Node 22 运行包、PostgreSQL 18.6、原数据库卷与 14 项 migration ledger 均保留，API/DB healthy、Web running；标准镜像仅完成 Compose 配置验证，没有实际启动。

## 4. 外部与平台集成

- Apple StoreKit、App Attest、App Store Server Notifications V2 与 Server API。
- Google/Apple OAuth；邮箱注册与找回密码使用 ZeptoMail。
- Scan 使用端侧 RTMDet-Ins 检测、原生透视矫正和 Dart RGB pHash：iOS 16+ 使用 Core ML/Core Image，Android 使用最小 ONNX Runtime 与 Bitmap。PE-Core-T16 模型源文件移至未打包目录，双端运行时不再调用；Android AAR 仍是原两模型算子集，未在本次重建。App 不包含或运行 ML Kit Latin OCR，不再提交端侧卡号提示；Web 暂不支持端侧扫描。
- dev Linux 配置以必填 `VECTOR_RECOGNITION_BASE_URL` 构造 HTTP `VECTOR_RECOGNITION`，只发送 `{r,g,b,game_id?}` 给 pHash 识别服务；10 秒超时覆盖正文，业务数据库和图片卷保持本地。旧 OCR 字段已退出 dev 运行路径；prod 保持原运行版本，协议切换须独立发布和验收。
...
- Flutter 通过 `--dart-define-from-file` 选择环境，`APP_ENV=test` 默认业务 API 为 `http://192.168.50.201:8080/api/v1`，production 和未配置值维持原生产 HTTPS API。Android 从同一参数选择指定 IP 的测试 HTTP 策略，iOS 由现有 test flavor 的独立 plist 配置；根 Melos 测试显式注入 `APP_ENV=test`。
