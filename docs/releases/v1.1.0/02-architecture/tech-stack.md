...
| 数据访问 | Drizzle ORM、D1、顺序 SQL migrations | `src/db/schema.ts`、`src/db/migrations/` |
...
- OCR 服务处理扫描识别；D1/R2 保存结构化记录与受保护图片。
...
- dev 与 prod 的 Bundle ID、Product ID、D1/KV/R2 和密钥严格隔离。
