...
- 当前代码与部署基线：2026-09-23 Linux API/Admin 运行 `dev@f9feac7`，包含请求关联、Admin 内网 HTTP UUID fallback 和 Sports Shop eBay 入口；远端后续 `dev@d8f0aa2` 仅更新 Flutter 版本与文档，未替换 Linux release。客户端源码版本为 `1.0.3+158`，此前已构建的测试包不包含本次 Sports Shop UI，移动端业务仍需新包真机验收。`dev-inner@b941a3f` 与 `dev@8e22c1d` 分别保留为 2026-09-15 整改和 2026-08-26 原始设计基线。
