...
`dev-xiangyang-new` 试验分支在不改变 Linux HTTP 适配器和数据库/图片存储边界的情况下，将共享路由改为接受 RGB pHash 并向上游只发送 `{r,g,b,game_id?}`。该分支的 `VECTOR_RECOGNITION_BASE_URL` 应为 `https://recognize.tcgcard.fun`，App 与 API 需同步发布；下述 512 维向量链路和历史服务器验证仅属于 `dev` 基线，本试验尚未部署或经真机验证。
