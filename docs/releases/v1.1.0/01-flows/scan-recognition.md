...
当前 main 为 `2cfdea8`，已合入 `dev@ad88ee9`，App 为 `1.0.3+150`。prod Cloudflare 通过 Service Binding、dev Linux 通过 HTTP 适配调用 `recognize-vec`；2026-09-21 prod 已从当前 main 重新发布 version `c612c8a6-4873-4760-b435-3c4db27d14e6`，dev Linux API 实际运行 `dev@9488a15`。下方来源提交和历史版本保留原日期，不能据此推断客户端或 Linux release 随 Git 合并改变。
