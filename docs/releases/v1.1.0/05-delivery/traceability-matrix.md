...
| [PostgreSQL 查询切换与页面数据语义](../03-data-api/contract-changes.md#postgresql-查询切换) | [PostgreSQL 运行入口](../../../../apps/workers-api/src/index.ts)、[价格查询](../../../../apps/workers-api/src/data-source/postgres-price-store.ts)、[目录/价格路由](../../../../apps/workers-api/src/data-source/routes.ts)、[Portfolio](../../../../apps/workers-api/src/portfolio/valuation-history.ts)、Flutter Home/Search/Collection/Card Detail | `index-postgres-runtime.test.ts`、`postgres-price-store.test.ts`、`local-db-adapter.test.ts`、`routes.test.ts`、`valuation-history.test.ts`、Flutter 页面 Controller/Widget 测试 | 代码已完成：运行时只允许 Hyperdrive/PostgreSQL；D1 完全独立且不得查询、补全或回退；页面区分成功空数据、缺价、合法零价和查询失败。价格实数依赖独立上游导入，迁移检查点七张价格表为空不属于客户端伪造数据的理由。 |
...
dev 当前边界（2026-08-18）：Apple Sandbox URL、正式 PostgreSQL Worker/Admin、Bundle、共享 PostgreSQL 通知表与重试任务均已就绪；迁移的 34 条 inbox 均属于 `Sandbox`。Root CA 已用 Apple 官方 G3 指纹核验输入更新并生效，但尚无更新后的真实 Apple Sandbox 通知端到端证据，Server API Secret 仍需真实调用验证。prod 只创建了 Root CA 待部署版本，线上 deployment/version 和流量未变化。
