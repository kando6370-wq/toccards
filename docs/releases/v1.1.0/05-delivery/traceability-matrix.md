...
| [Admin Apple Notifications V2](../00-product/TCG_Admin_%E8%AE%A2%E5%8D%95%E7%BB%9F%E8%AE%A1%E4%B8%8E%E8%8B%B9%E6%9E%9C%E9%80%9A%E7%9F%A5%E6%B6%88%E6%81%AF_PRD_V1.1_%E8%AE%A2%E9%98%85%E7%9C%9F%E5%80%BC%E6%94%B6%E5%8F%A3%E7%89%88.md#11-apple-通知与订单业务处理关系) | [接收与归约](../../../../apps/workers-api/src/entitlements/apple-notification-routes.ts)、[Server API 校正](../../../../apps/workers-api/src/entitlements/apple-server-api-correction.ts)、[Admin Web](../../../../apps/admin-web/src/App.tsx)、迁移 `0029` | `apple-notification-routes.integration.test.ts`（含未知请求字段/通知类型前向兼容）、`apple-server-api-correction.integration.test.ts`、`billing-routes.integration.test.ts`（组合筛选、失败记录倒序分页、非法时间范围）、`billing-admin-intent.test.mjs`（第 14/15 章展示与两列详情布局） | 代码已完成，外部验收待完成：2026-08-17 已将 34 条 dev inbox 迁入共享 PostgreSQL 并固定为 `Sandbox`，正式 dev Worker/Admin 与 5 分钟重试已部署；Root CA/Server API Secret 配置项及 Sandbox URL 已就绪。Secret 内容有效性、真实 Sandbox 通知、退款/恢复实单及 production 配置仍待验证。 |
...
dev 当前边界（2026-08-17）：Apple Sandbox URL 已保存，正式 PostgreSQL Worker/Admin、Bundle、Root CA/Server API Secret 配置项、共享 PostgreSQL 通知表与重试任务已就绪；迁移的 34 条 inbox 均属于 `Sandbox`，但尚无真实 Apple Sandbox 通知端到端证据。配置项存在不证明加密 Secret 内容有效；prod 未部署，本次只读复核确认其 deployment/version 未变化。

1. 提供并冻结 production Apple Product ID、Bundle/App ID、Root CA、App Store Server API Secret 与 Singular Key。
