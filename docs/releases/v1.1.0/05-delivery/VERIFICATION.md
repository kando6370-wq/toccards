...
本轮没有 prod/dev 部署、远程 migration、缓存清理、生产认证请求、Mixpanel 写入、Git commit/push 或 main 合并。新认证与额度改动本身无 Schema 依赖；Search 要在 prod 生效仍须另行授权，按迁移文档先并发预建/核验 `0013`，再合并和发布。由于尚未部署，不能宣称线上接口已控制在 1 秒内；prod 发布后必须对相同冷/热 Search、当前认证接口、Quota 与真实 Scan 做前后 wall time 对照，并回读 1 秒阶段日志。业务/API 文档影响为 N/A：响应、授权、额度、扫描和估值契约均未变化。
