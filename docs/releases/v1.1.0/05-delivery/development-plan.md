...
- Restore 失败诊断已保留非敏感原生错误元数据：iOS `AppStore.sync()` 异常通过 `FlutterError.details` 返回 `NSError.domain/code`，Flutter 记录异常与堆栈，同时继续使用既有 Failed UI 和状态判定；日志不包含账号、Product ID 或交易 JWS。具体真机 StoreKit 错误码仍需安装新构建后复现确认。
- Restore 的 15 秒 Deadline 已移到 `AppStore.sync()` 完成后的 `Transaction.currentEntitlements` 读取阶段；Apple 系统账号/密码交互不再计入 Deadline，避免用户仍在输入密码时提前显示 Failed。同步取消或 StoreKit 异常继续显式失败，权益读取仍保留 15 秒超时保护；Android 订阅路径不受影响。
