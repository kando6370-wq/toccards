...
- Admin 订单与通知已按通知真值收口：Fresh Purchase/Restore 的服务端验签交易仅作为即时权益和交易暂存证据，`source_notification_uuid` 为空时不进入 Admin 列表、筛选选项、XLSX 或扣款编号；已验签、已解码的建单通知到达同一 `environment + transactionId` 后用通知字段晋升暂存记录。PostgreSQL `0000`/`0007` 提供订单事实与退款前业务状态；试用为 0，通知确认成功付费按 `environment + originalTransactionId`、`purchase_at + transactionId` 确定性编号，重复交易不增加次数，退款保留原序号，`REFUND_REVERSED` 仅在 Server API 校正为 active 后恢复。共享 PostgreSQL Schema 已于 2026-08-19 应用 `0007` 并完成幂等复核，dev Worker 与 Admin assets 部署待本次后续执行。`DID_CHANGE_RENEWAL_PREF + UPGRADE/DOWNGRADE` 只记录下一 Product ID、不建单；`SUBSCRIBED + RESUBSCRIBE` 使用新链时为首次付款/次数 1，沿用旧链时重算为续费。已验签建单通知可创建 `unlinked` 链和订单但不创建 grant。订单页按 PRD 展示 13 列和 11 项筛选，导出当前筛选的全部通知确认订单为 XLSX。通知页使用可搜索单选主/子类型联动，补齐请求期禁用、越界回退、固定空/失败状态；详情按需加载，包含独立 Loading/失败重试、两列基本信息和深色 JSON。真实 Sandbox 通知、退款撤销、自动续订快照、升级/降级、重订阅与后续补关联仍待验收。
