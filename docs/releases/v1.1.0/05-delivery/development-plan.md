...
| Billing 数据 | `0025` 至 `0034` 已覆盖购买链、交易、session grant、原始通知 inbox、生命周期、订单业务事实、USD 汇率及自动续订订单快照；对应结构与 dev 数据已迁入共享 PostgreSQL，34 条 inbox 均归属 `Sandbox` | prod Worker 尚未部署 PostgreSQL 版本；缺 Sandbox/TestFlight 实单验收 |
| Scan | 服务端已有终身 10 次真源、request ID 原子预占、逐张结算/返还、60 秒租约、响应重放和多设备并发保护；Quota 查询、识别成功及额度耗尽均返回完整 `access/unlimited` 权益字段，Flutter 严格解析并合并本机 Premium 与服务端 Unlimited，完成 Waiting、Quota=0 Paywall、Done 公式、服务端确认 Unlimited 后自动递补、Scan Pro 完整订阅来源返回及 Processing 删除后的后台结算；扫描相机页顶部操作栏已收紧为 32px，并保持退出、闪光和搜索按钮垂直居中，状态栏实色背景仅覆盖系统安全区、不再覆盖其下方 10px 间距；扫描定位框及其识别裁剪区域已从页面顶部 `163px` 先下移至 `193px`，本次继续同步下移至 `213px`；定位框现以 `390x844` 下 `280x400、top=213` 为不变基准，在 iOS/Android 竖屏小屏设备上按安全区及顶部、底部操作区之间的可用空间连续缩小和上移，大屏不放大，黄色四角、径向遮罩、扫描线与 OCR 裁剪共用同一响应式矩形；初始、拍照、识别和结果揭示阶段统一保留黄色四角与框内透明效果，框外改为从定位框中心向屏幕边缘逐渐加深的径向暗化，移除识别/揭示阶段白色柔光，可见遮罩绘制层在相机、识别和揭示状态间保持同一实例以避免切换闪烁，顶部与底部操作区也在识别/揭示状态切换时持续显示、不再执行二次入场动画，扫描线仍仅在点击拍照后按原 500ms 时序执行一次；剩余扫描次数入口已同步 Figma `1644:6741` 的 48px 提示条、24px PRO 徽章及 13/16 两行文案样式，并置于扫描页最高层级，避免与取景框重叠时丢失点击 | Sandbox/TestFlight 与真实并发、超时规模验收尚未完成 |
...
### 当前开发检查点（2026-08-13，2026-08-17 补充核验）

- dev 发布已完成：远程 D1 连续应用 `0025` 至 `0034` 并确认无待迁移；首次 Worker 发布被 Cloudflare `10021` 拒绝，原因是 Apple 官方库的 `jsrsasign` 依赖在模块全局作用域生成随机值。验签器与 Server API Client 改为请求/定时任务内动态加载后，Workers 类型检查、Apple 权益/Restore/通知/校正定向 43 项及真实 Cloudflare 启动校验通过。dev/test Product ID 后续已确认为 `cardx.week`、`cardx.year`、`cardx.lifetime`，远程 dev D1 已写入三个 active `performance_pro` 映射，包含该白名单的 Worker/Admin 已部署为 `7228b912-c57f-43a3-8f2c-2404f8dac7bc`，`/api/v1/health` 返回 HTTP 200。该 2026-08-13 检查点中 Root CA 与 Server API 密钥仍缺失；较新的 dev 配置状态见下一项。prod Product ID 保持未配置，prod 未迁移、未部署。
...
- App PRD 验收 125-128 已形成直接自动化证据：真实 Miniflare D1 测试证明两个 session 并发创建 Folder 只能一个成功且总数保持 2；ATT 测试证明后台恢复只读取状态并同步 Singular、不再次请求授权，未完成 Onboarding 的后续冷启动也由独立安装 marker 阻止重复请求；存量安装升级只认领 marker 并同步当前状态，不主动弹框。Scan Widget 测试分别证明 quota=0 的 Capture 与 Gallery 在 typed Premium success 后只返回原 Scan 页面，`photo`/`library` 调用次数仍为 0，用户必须重新操作。复验结果为 Scan Widget 81 项、Folder D1 集成 3 项、ATT 8 项及 `flutter analyze` 全部通过；这不替代 iOS 真机 ATT、Sandbox/TestFlight 和真实多设备验收。
...
- 配置边界：dev/test Product ID、共享 PostgreSQL 中迁移的 dev 商品映射、Workers dev 白名单、Apple Root CA/Server API Secret 配置项及 Sandbox 通知 URL 均已就绪；Secret 内容有效性和真实 Sandbox 通知仍待端到端验证。prod Product ID 仍按 App PRD 标记为 Pending Configuration，production/prod 白名单与客户端配置必须保持未配置。
...
当前状态：代码与 dev PostgreSQL 部署闭环，外部发布验证待完成。Notifications V2 接收、先存原文、官方验签、幂等、确定性归约、乱序保护、补偿重试、Apple Server API `correction_required` 消费与退款撤销校正已落地；dev 接收路由、共享 PostgreSQL 表、重试任务、Secret 配置项和 Sandbox URL 已就绪，但 Secret 内容有效性及完整 Sandbox/TestFlight 通知矩阵仍待验证，production 配置与部署需独立完成。
...
- 交易时点 USD 汇率快照代码已落地 `0033`：保存 rate、base/quote、来源、生效/抓取时间、陈旧标记、版本和舍入模式；汇率缺失不阻断订单且不伪造 USD 金额。该 D1 迁移曾在隔离空库和远程 dev 顺序执行；对应结构与 dev 数据现已迁入共享 PostgreSQL，prod Worker 尚未部署 PostgreSQL 版本。
...
- 苹果通知详情已按 PRD 15.2 使用桌面两列、小屏单列的基本信息布局；真实 D1 集成测试直接覆盖通知组合筛选、失败 inbox 参与创建时间倒序分页及非法时间范围显式拒绝，避免仅凭路由存在宣称查询闭环。

- 已补齐订单和通知字段、查询、正确扣款次数、安装时间、当前订阅状态和 XLSX 全量导出；真实 D1 集成测试逐项保护订单筛选的 AND 语义、有效订单时间排序、退款事实保留、列表/导出筛选一致性及非法筛选显式失败。
