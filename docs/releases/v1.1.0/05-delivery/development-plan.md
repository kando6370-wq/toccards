...
- Admin 订单与通知已落地：`0032` 增加 `business_status`、`charge_count` 与来源通知 UUID；试用为 0，成功付费按 `environment + originalTransactionId`、`purchase_at + transactionId` 确定性编号，重复交易不增加次数，退款保留原序号。`0034` 将自动续订改为订单事件快照：Notifications 有已验签续订信息时写 `0/1`，Lifetime 固定 `0`，无法证明的 Fresh Purchase、Restore 与历史订阅保持空值；Admin 展示、筛选和导出不再读取链路当前值。`DID_CHANGE_RENEWAL_PREF + UPGRADE/DOWNGRADE` 只记录下一 Product ID、不建单；`SUBSCRIBED + RESUBSCRIBE` 使用新链时为首次付款/次数 1，沿用旧链时即使 Apple 原因为 `PURCHASE` 也重算为续费/次数递增。已验签建单通知在没有 UID/既有 chain 时可按启用 SKU 建立 `unlinked` 链和订单，Admin 显示空 UID且精确 UID 筛选不误匹配；通知不创建 grant，未知 SKU 隔离，后续只有可信 Fresh Purchase/Restore session proof 可补 owner。订单页按 PRD 展示 13 列和 11 项筛选，国家/SKU 来自实际数据，导出当前筛选的全部结果为真实 XLSX；列表与导出复用同一筛选契约，全部条件按 AND 组合，非法日期、扣款次数或自动续订值显式返回 `422`。退款订单按退款完成时间参与订单时间倒序，同时保留原交易金额和退款时间事实；同步导出上限 10,000 行，超限显式失败。通知页以 inbox 为入口，验签/解析/业务失败记录可见，列表不加载完整 JSON，详情不返回 `signedPayload`，仅在授权用户主动打开详情后加载 Decoded Payload，并由用户主动复制后显示成功 Toast。最新 Admin PRD 未要求新增查看/复制审计；真实 Apple 无 UID通知、自动续订快照、升级/降级、重订阅与后续补关联仍待 Sandbox 验收。
...
- 苹果通知消息页已按 PRD 第 14 章收口固定说明、筛选顺序、列表标题和缺失值 `--` 展示；动态主类型/子类型联动及未知类型兼容保持不变。
- 苹果通知消息页的创建时间筛选已支持日期时间范围，刷新入口位于通知消息列表卡片右上角。
- 苹果通知详情已按 PRD 15.2 使用桌面两列、小屏单列的基本信息布局；本地 Miniflare D1 兼容层集成测试直接覆盖通知组合筛选、失败 inbox 参与创建时间倒序分页及非法时间范围显式拒绝，避免仅凭路由存在宣称查询闭环；该测试不代表运行时读取 D1。

- 已补齐订单和通知字段、查询、正确扣款次数、安装时间、当前订阅状态和 XLSX 全量导出；本地 Miniflare D1 兼容层集成测试逐项保护订单筛选的 AND 语义、有效订单时间排序、退款事实保留、列表/导出筛选一致性及非法筛选显式失败；该测试不代表运行时读取 D1。
