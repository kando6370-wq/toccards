...
| Billing 数据 | `0025` 至 `0034` 已覆盖购买链、交易、session grant、原始通知 inbox、生命周期、订单业务事实、USD 汇率及自动续订订单快照；自动化完整迁移链已通过 `0000-0034`，远程 dev 已连续执行并确认无待迁移 | prod 与常用 local 尚未执行；缺 Sandbox/TestFlight 实单验收 |
| Scan | 服务端已有终身 10 次真源、request ID 原子预占、逐张结算/返还、60 秒租约、响应重放和多设备并发保护；Flutter 已使用服务端 quota 真值，完成 Waiting、Quota=0 Paywall、Done 公式、服务端确认 Unlimited 后自动递补、Scan Pro 完整订阅来源返回及 Processing 删除后的后台结算；扫描相机页顶部操作栏已收紧为 32px，并保持退出、闪光和搜索按钮垂直居中，状态栏实色背景仅覆盖系统安全区、不再覆盖其下方 10px 间距；剩余扫描次数入口已同步 Figma `1644:6741` 的 48px 提示条、24px PRO 徽章及 13/16 两行文案样式，并置于扫描页最高层级，避免与取景框重叠时丢失点击 | Sandbox/TestFlight 与真实并发、超时规模验收尚未完成 |
| Folder | 已有 Folder CRUD；Free 总数最多 2 个（含默认 Folder），服务端按当前 session grant 判断，并以单条条件 INSERT 防止并发越限；购买或恢复成功后在原 Folder Sheet 仍有效时打开 Create Folder Modal；服务端并发上限拒绝会先刷新 Folder List 再进入 Paywall，普通失败或权益同步中保留名称输入 | dev 已执行相关迁移；仍缺 Sandbox/TestFlight 多设备人工验收 |
...
- 2026-08-17 dev Apple 回调补充核验：App Store Connect Sandbox Server URL 已保存为 `https://api-dev.tcgcard.fun/api/v1/apple/notifications/v2`；当前 `toccards-api-dev` 部署对应提交 `df8e6d7c8ae6695d3f0366b17725abd65615e8d8`，`APP_ENVIRONMENT=development`、`APPLE_IAP_BUNDLE_ID=com.kando.kandoApp.beta`，Apple Root CA 与 App Store Server API Issuer ID/Key ID/Private Key 的 Secret 配置项均存在。远程 dev D1 的 `apple_notification_inbox`、`apple_server_notification` 及 5 分钟重试任务已就绪，inbox 当前为空。空 JSON 请求返回 `400 INVALID_REQUEST` 只证明请求到达业务校验；Secret 内容有效性、真实 Apple Sandbox 通知验签、入库与业务处理仍未端到端验证。本次未核验 production。
...
- 配置边界：dev/test Product ID、dev D1 商品映射、Workers dev 白名单、Apple Root CA/Server API Secret 配置项及 Sandbox 通知 URL 均已就绪；Secret 内容有效性和真实 Sandbox 通知仍待端到端验证。prod Product ID 仍按 App PRD 标记为 Pending Configuration，production/prod 必须保持未配置。
...
当前状态：代码闭环，发布验证待完成。Notifications V2 接收、先存原文、官方验签、幂等、确定性归约、乱序保护、补偿重试、Apple Server API `correction_required` 消费与退款撤销校正已落地；dev 接收路由、D1 表、重试任务、Secret 配置项和 Sandbox URL 已就绪，但 Secret 内容有效性及完整 Sandbox/TestFlight 通知矩阵仍待验证，production 配置需独立完成。
...
- 交易时点 USD 汇率快照代码已落地 `0033`：保存 rate、base/quote、来源、生效/抓取时间、陈旧标记、版本和舍入模式；汇率缺失不阻断订单且不伪造 USD 金额。迁移已通过隔离空库顺序执行并应用到远程 dev，尚未应用到常用 local 或 prod。
