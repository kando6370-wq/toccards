...
| [App 归因与收入上报](00-product/TCG_Card_App_v1.1_PRD.md#17-analytics) | [ATT/Singular](../../../apps/flutter-app/lib/shared/attribution/app_attribution.dart)、[收入上报](../../../apps/flutter-app/lib/features/subscription/subscription_revenue_reporter.dart) | `app_attribution_test.dart`（独立安装 marker、未完成 Onboarding 重启不重复请求、存量升级不弹框、后台只读同步）、`onboarding_gate_test.dart`（marker 不延长存量启动门禁）、`subscription_revenue_reporter_test.dart`（verified-only、实时/启动并发幂等、失败重试与损坏存储拒绝） | 代码已完成，外部验收待完成：Singular 正式 Key、iOS 真机 ATT 与 Sandbox Revenue。 |
