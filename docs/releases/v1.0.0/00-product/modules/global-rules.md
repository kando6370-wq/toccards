...
>
> **相关契约**：
> - API 规范 [`docs/03-data-api/api-spec.md`](../../03-data-api/api-spec.md)
> - 模块 PRD：[`auth.md`](./auth.md)、[`card-detail.md`](./card-detail.md)、[`collection.md`](./collection.md)、[`home.md`](./home.md)、[`profile.md`](./profile.md)
...
**固化结论**：
- **数据来源**：以当前 D1 卡牌基础表为准，App 不自行维护价格数据库；卡牌价格、历史价格序列均来自 `tcg_price`（Raw 使用 `price_Ungraded`，评级价格使用对应 `price_*` 字段），经 Workers 数据源适配层返回（见 `third-party.md`）。
- **默认排序**：Search / 系列列表等默认排序不再以"客户端本地入库时间"为业务口径，改为使用后端数据源适配层返回的默认排序字段（如 `updated_at`、`release_date` 或采集程序维护的排序字段）；具体排序字段由接口返回，前端按接口结果展示。
