...
> **最后更新**：2026-06-30
> **维护口径**：本文档是整理后的术语契约，不再依赖仓库内的历史设计稿。
...
**中文释义**：今日趋势榜。v1.0 先展示运营置顶卡牌；非置顶趋势取决于本地基础表后续是否提供趋势算法或采集数据。

**用法**：不受当前选中文件夹影响，不要求用户收藏；Home 首页展示 3 条；`trending_pin` 置顶项按 `rank` 优先展示并回查 `cards_all`；若本地基础表暂无算法 Trending，非置顶列表可为空；涨跌幅不随货币切换变化。
...
**中文释义**：卡牌数据覆盖层。存储于 D1，用于补充基础表缺失的卡牌、纠正数据错误、补充卡牌图片、存放运营数据（如 Trending 置顶）。

**用法**：读取卡牌数据时，覆盖层优先，无覆盖数据则回落 D1 卡牌基础表；管理后台"卡牌数据运维"模块负责维护覆盖层；App 端和管理后台均不直连采集程序，统一经 Workers 代理。
