...
> - 第三方适配层 [`../03-data-api/third-party.md`](../03-data-api/third-party.md)
...
| M2 | 数据代理层 | M0 | 第三方 API 适配层 + KV/Cache 缓存 + 降级 |
...
| M0-7 | CI 流水线 | GitHub Actions 双 Job：TS 侧（`pnpm turbo build/lint/type-check`）+ Dart 侧（`melos run build/analyze`），参见 [`tech-stack.md §2.5`](../02-architecture/tech-stack.md) |
...
**目标**：实现 Workers 数据代理层，对接第三方卡牌数据源（TCGplayer / eBay / PriceCharting 等），提供搜索、价格、Trending、成交记录等接口，并实现 KV/Cache 缓存与降级逻辑。
...
| M2-2 | 第三方厂商适配实现 | 接入选定厂商（⚠️ TBD M2-A）；实现 `DataSourceAdapter` 接口的具体适配类 |
...
| M2-5 | 降级兜底逻辑 | 第三方和缓存均失败时返回空数组；客户端展示 "No content available"（见 [`api-spec.md §4`](../03-data-api/api-spec.md) 各端点降级说明） |
...
- 使用 Mock 适配器时，`GET /cards/search?q=charizard` 返回结构化 JSON，分页正确。
- 缓存命中时不再请求第三方（可通过日志或测试用例验证）。
- 第三方失败时接口仍返回 200，`items: []`，无 500 崩溃。
- `card_override` 有记录的 card_ref 返回的数据中 `override_applied: true`，字段已合并。
- 汇率接口有响应（即使 Mock），结构符合 `api-spec.md §4.8`。

### 依赖与阻塞

- ⚠️ **TBD M2-A**：第三方数据源厂商选定（TCGplayer / eBay / PriceCharting 等）——阻塞 M2-2 真实适配实现。开发阶段用 Mock 适配器推进，厂商确定后替换适配类，接口契约不变（见 [`third-party.md`](../03-data-api/third-party.md)、[`api-spec.md §6 #4`](../03-data-api/api-spec.md)）。
- ⚠️ **TBD M2-B**：汇率接口提供方——阻塞 M2-7 真实接入（见 [`api-spec.md §6 #3`](../03-data-api/api-spec.md)、[`tech-stack.md §3`](../02-architecture/tech-stack.md)）。
- ⚠️ **TBD M2-C**：各接口最终 TTL（取决于厂商限速策略）——上线前须基于厂商文档确认（见 [`api-spec.md §6 #9`](../03-data-api/api-spec.md)）。
...
- ⚠️ **TBD M3-A**：`card_ref` 格式取决于第三方厂商（影响 `collection_item.card_ref`、`wishlist_item.card_ref` 的实际值，但接口结构不变）。见 [`api-spec.md §6 #4`](../03-data-api/api-spec.md)。
...
| M8-3 | 第三方数据源联调 | 将 Mock 适配器替换为选定厂商的真实适配实现（解除 TBD M2-A）；验证搜索/价格/Trending 真实数据返回 |
...
| M8-8 | TTL / 限速确认 | 基于厂商文档确认各代理接口最终 TTL（解除 TBD M2-C）；调整 KV/Cache TTL 配置 |
...
| TBD M2-A | 第三方数据源厂商选定 | M2、M8 | 开发用 Mock 适配器；厂商选定后替换适配类；见 [`third-party.md`](../03-data-api/third-party.md)、[`api-spec.md §6 #4`](../03-data-api/api-spec.md) |
...
| TBD M2-C | 各代理接口最终 TTL | M2、M8 | 取决于厂商限速策略；M8 前基于厂商文档确认；见 [`api-spec.md §6 #9`](../03-data-api/api-spec.md) |
