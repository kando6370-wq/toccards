...
4. **覆盖层优先**：Workers 返回卡牌数据时，先读 D1 `card_override` 表，有覆盖字段则合并后返回，无覆盖则直接返回第三方数据（见数据模型 §5.1）。
...
|---|---|---|
| Raw 当前市场价 | `MarketPrice.price`（grader='Raw'） | 按对应 condition 取价 |
| Graded 当前市场价 | `MarketPrice.price`（grader+grade） | 按 grader + grade 组合取价 |
| Sealed 当前市场价 | `MarketPrice.price`（grader='Raw'，Sealed 对象） | Sealed 无评级，取 Raw 价 |
