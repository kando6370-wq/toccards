...
| 生产价格历史覆盖率不足 | JustTCG Raw 同步程序与 Cron 已部署，但生产缺少 `JUSTTCG_API_KEY`，状态为 blocked；4066 个目录 product 中仍仅 10 个存在 SKU 价格，最新日期为 2026-07-08 | 不插测试价格掩盖；配置至少 Starter 配额并跑完真实回填前仍为 P0 数据管线上线依赖 |
