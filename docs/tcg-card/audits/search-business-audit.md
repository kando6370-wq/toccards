...
| Sets 浏览/搜索 | `GET /sets/search?q=&game=` | 读取真实 `sets` 表；封面由 `sets.product_id` 映射 R2 |
...
| `sets` | game、name、set_code、product_id、total_cards | Set 列表、R2 封面与卡数来源 |
...
| 生产 Set 数据 | `sets` 共 4239 行，50 行已有 `product_id`；重复 `set_name` 已迁移删除，API 仍由 `name` 映射 `set_name` |
