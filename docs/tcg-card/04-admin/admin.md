...
后台鉴权基于 `admin_user` 表（见 data-model.md §5.1），由独立的 Admin Token（JWT）签发。
...
后台 UI 文案面向内部运营团队，采用**中文**（如本文中的弹窗标题、按钮、空状态文案）；App 端面向海外用户采用**英文**（引用 `global-rules.md §十三` 的英文文案表）。两者语言不同为有意选择。
...
**关联表**：`feedback_ticket`（data-model.md §6.4）
...
**关联表**：`app_config`（data-model.md §6.3）、`trending_pin`（data-model.md §6.2）
...
**关联表**：`card_override`（data-model.md §6.1）
