...
- 修改 `quantity` 时，`quantity` 必须 ≥1 且为整数，否则不允许保存；校验文案见 [global-rules §13.1](./global-rules.md#131-通用文案全局生效)（`Quantity must be at least 1.` / `Quantity must be a whole number.`）。
...
- `quantity` 修改后，从保存时间点起按新数量计算资产，历史**不回写**（见 [global-rules §16.2](./global-rules.md#162-资产变更只影响变更时间点之后全局原则)）。
- Wishlist 不展示数量。
...
| Portfolio / Wishlist 互斥不变量 | 同一张卡不可同时存在于 Portfolio 与 Wishlist（口径见 [global-rules §16.7](./global-rules.md#167-wishlist-与收藏状态口径)）；加入 Portfolio 时自动从 Wishlist 移除的流程见上一条与[§9.3](#93-wishlist-加入-portfolio) |
