...
>
> **上游来源**：
> - 原始底稿 [`docs/tcg_cord_docs/home页说明.md`](../../../tcg_cord_docs/home页说明.md)
...
- 再次点击后恢复显示。
- **Home / Collection 金额隐藏状态联动同步**：用户在 Home 隐藏资产金额后，进入 Collection - Portfolio，金额同步隐藏；反之亦然。联动通过 `user_preference.amount_hidden` 字段（`PATCH /preferences`）持久化，游客账号同样生效（见 [global-rules §七](./global-rules.md#七金额与百分比规则) 和 [data-model §4.4](../../03-data-api/data-model.md#44-user_preference用户偏好)）。
...
- 相同单张价值时排序规则：① 30 天涨幅较高者优先；② 涨幅相同时最近添加时间较晚者优先（引用 [global-rules §十五 冲突2](./global-rules.md#冲突-2most-valuable-相同单张价值时的排序优先级)）。
...
- 冷启动时，Home 默认展示**星标文件夹**（`portfolio_folder.is_default = 1`）数据。
- 如果 `user_preference.last_selected_folder_id` 非空（用户上次手动切换过），则继续沿用该文件夹，直到下一次冷启动才回归星标文件夹（冷启动重置 `last_selected_folder_id` 语义）。
