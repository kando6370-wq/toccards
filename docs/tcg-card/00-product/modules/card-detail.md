...
>
> **上游来源**：
> - 原始底稿 [`docs/tcg-card/source-tcg-card-docs/卡牌详情.md`](../../source-tcg-card-docs/卡牌详情.md)
...
8. 卡牌不可同时存在于 Wishlist 和 Portfolio 中，Remove from Portfolio 不造成 Wishlist 的影响。
...
2. Collection Item 表单**不展示** Grader / Grade、Condition、Language、Configuration 等扩展字段。Product Type 作为后端 CI 存储字段（由列表对象数据带入，见 `search.md §15.3`），但不在表单 UI 中展示给用户编辑。
...
6. Market Price 优先基于成交价格计算，不直接使用单个在售挂牌价作为资产估值；Shop 列表可展示在售商品，但不等同于资产估值价格（见 `global-rules.md §17.1`）。
7. Market Prices 表格的 7D Change 为百分比，不受货币切换影响（见 `global-rules.md §七`）。
8. 公共数据不可用时，Price Tab 不按普通加载失败处理（区别于局部失败的 `No content available`），展示专用三行文案（见 §11.1）。
...
### 9.7.1 Language / Finish / Variant 取价规则

1. Language、Finish、Variant 同为价格取值字段；不同取值不合并估值（见 `global-rules.md §17.2`）。
2. 用户修改上述任一取值后，从保存时间点起按新取值计价，历史不回写。
3. 所选取值无市场价时，当前价值展示 `--`，仍允许保存。
...
6. 卡牌不可同时存在于 Wishlist 和 Portfolio 中，Remove from Portfolio 不造成 Wishlist 的影响。
...
公共卡牌后台变更不直接删除用户已收藏的 Collection Item，按公共卡牌状态分别处理（口径见 `global-rules.md §十九`，下表为本模块简述）：

| 状态 | 对 Collection Item 的处理 |
|---|---|
| **Delisted** | 卡牌从 Search 公共列表下架，但详情页与 Collection Item 继续展示；价格仍可用则继续计价，不可用则展示 `--`、不计入 Home 总资产和 Most Valuable（见 `global-rules.md §19.1`） |
| **Merged** | Collection Item 自动关联到新的 canonical card_id，用户持有信息保持不变，后续价格和基础信息使用新 card_id（见 `global-rules.md §19.2`） |
| **Unavailable** | Collection Item 不删除，展示最近一次可用基础信息缓存；当前价格 `--`、涨跌幅 `-/-`，从异常开始不计入 Home 总资产和 Most Valuable；用户仍可编辑或移除（见 `global-rules.md §19.3`） |

公共数据不可用时，Price Tab **不按普通加载失败处理**，展示专用三行文案（区别于局部失败的 `No content available`）：
...
Unavailable 后的历史快照与图表口径（已生成快照保留、之后不再计入、恢复 / 合并后重新参与）见 `global-rules.md §19.4`。
